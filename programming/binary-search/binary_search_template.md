...
| [540. 有序数组中的单一元素](0540.有序数组中的单一元素.cpp) | check = `nums[mid] == nums[mid^1]`，mid^1 取配对搭档，分界点即答案 |
| [1539. 第 k 个缺失的正整数](1539.第k个缺失的正整数.cpp) | check = `arr[mid] - mid - 1 >= k`，找分界后答案 = `k + left` |
| [1608. 特殊数组的特征值](1608.特殊数组的特征值.cpp) | 枚举 x，`n - lower_bound(nums, x)` 计数 |
| [1287. 有序数组中出现次数超过25%的元素](1287.有序数组中出现次数超过25%的元素.cpp) | 检查 3 个候选位置，`upper_bound - lower_bound` 验证 |
...
| [153. 寻找旋转排序数组中的最小值](153.寻找旋转排序数组中的最小值.cpp) | 以 `nums.back()` 为固定基准，`nums[mid] > 基准` → 红（左段），否则蓝 |
| [154. 寻找旋转排序数组中的最小值 II](154.寻找旋转排序数组中的最小值-ii.cpp) | 先跳过尾部与 `nums[0]` 相等的元素锁定 end，再套 153 模板（基准换成 `nums[end]`）|
...
| 题目 | 要点 |
|------|------|
| [875. 爱吃香蕉的珂珂](0875.爱吃香蕉的珂珂.cpp) | 二分速度 k，check = 能否 h 小时吃完 |
| [1011. 在 D 天内送达包裹的能力](1011.在-d-天内送达包裹的能力.cpp) | 二分运力，check = 能否 D 天运完 |
| [410. 分割数组的最大值](0410.分割数组的最大值.cpp) | 二分最大和，check = 能否分成 m 段 |
| [1482. 制作花束](1482.制作花束.cpp) | 二分天数，check = 能否做成 m 束 |
| [1283. 使结果不超过阈值的最小除数](1283.使结果不超过阈值的最小除数.cpp) | 二分除数，check = 除完总和 <= threshold |
| [1802. 有界数组中指定下标处的最大值](1802.有界数组中指定下标处的最大值.cpp) | 二分峰值，check = 总和 <= maxSum |
| [1870. 准时到达的列车最小时速](1870.准时到达的列车最小时速.cpp) | 二分时速（浮点版，放大 100 倍转整数） |
| [2064. 分配给商店的最多商品的最小值](2064.分配给商店的最多商品的最小值.cpp) | 「最小化最大值」经典题 |
| [2226. 每个小孩最多能分到多少糖果](2226.每个小孩最多能分到多少糖果.cpp) | 「最大化最小值」经典题 |
| [2594. 修车的最少时间](2594.修车的最少时间.cpp) | 二分时间，check = 能修完 cars 辆 |
| [2560. 打家劫舍 IV](2560.打家劫舍-iv.cpp) | 二分窃取价值 + 贪心 check |
| [1760. 袋子里最少数目的球](1760.袋子里最少数目的球.cpp) | 二分最大球数 |
| [441. 排列硬币](0441.排列硬币.cpp) | 二分行数 |
| [1201. 丑数 III](1201.丑数-iii.cpp) | 二分答案 + 容斥计数 |
| [793. 阶乘函数后 K 个零](0793.阶乘函数后-k-个零.cpp) | 二分 n，check = 尾零数 |
| [69. x 的平方根](../math/0069.x-的平方根.cpp) | 二分答案，check = 平方 <= x |
| [367. 有效的完全平方数](0367.有效的完全平方数.cpp) | 69 的变体，找 floor(sqrt(num)) |
| [1891. 割绳子（会员题）](1891.割绳子.cpp) | 同 410 思路，「最大化最小值」 |
